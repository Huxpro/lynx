...
`enableGridLanes` was introduced in SDK 4.1 with a default of `false`. Keep the
...
Grid Lanes and `flow-tolerance` are available from SDK 4.1 on Android, iOS,
HarmonyOS, and Clay platforms. Lynx for Web uses the browser-native
implementation when supported and keeps its `@supports` fallback otherwise.
